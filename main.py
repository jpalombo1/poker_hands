...
from poker.common import CardsLeft, Suits, Values
...
def main():
    """Main function to play start, flop, river, turn for given number of hands.

    Make deck, generate hands from it for each player, play turn and calculate win probability until end of game.
    """
...
def test():
    """Special test cases to verify working functionality."""
    CASES = [
        [
            [
    ...
    for player_hands in CASES:
        deck = generate_deck()
        for player_hand in player_hands:
            deck = remove_from_deck(deck, player_hand)
        win_probs = [
            prob_win(player_hands, deck, CardsLeft.RIVER, pnum)
            for pnum in range(len(player_hands))
        ]
        print(win_probs)
